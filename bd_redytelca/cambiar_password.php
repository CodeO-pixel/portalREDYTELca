...
if ($userId > 0) {
    if ((int) $staffSession['id_rol'] !== 1) {
        http_response_code(403);
        echo json_encode(['status' => 'error', 'message' => 'Solo un Administrador puede restablecer la contraseña de otro usuario.']);
        exit;
    }
    ...
    if ($username !== $staffSession['username']) {
        http_response_code(403);
        echo json_encode(['status' => 'error', 'message' => 'No puedes cambiar la contraseña de otro usuario por esta vía.']);
        exit;
    }
...
// Corrección: el gate del archivo acepta cualquier staff válido y la rama de reset exige rol de Administrador únicamente allí, preservando el auto-cambio de Operador y cerrando el reset ajeno sin sesión.
