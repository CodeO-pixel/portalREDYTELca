...
function initToolbar(moduleName, options = {}) {
    const toolbar = document.querySelector(`[data-toolbar="${moduleName}"]`);
    if (!toolbar) {
        return;
    }

    const moduleButtons = toolbar.querySelectorAll('.toolbar-btn[data-module]');
    moduleButtons.forEach(btn => {
        const action = btn.dataset.action || '';
        if (action === 'edit' || action === 'delete' || action === 'context') {
            btn.disabled = true;
        }
    });
    ...
    if (options.searchId) {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', () => {
                updateToolbarButtons(moduleName);
            });
        }
    }
}

function selectRow(moduleName, id, rowElement) {
    const previousId = window.selectedIds[moduleName];
    if (previousId) {
        const previousRow = document.querySelector(`[data-module-row="${moduleName}"][data-id="${previousId}"]`);
        if (previousRow) {
            previousRow.classList.remove('selected-row');
        }
    }

    window.selectedIds[moduleName] = id;
    if (rowElement) {
...
function getSelectedId(moduleName) {
    return window.selectedIds[moduleName] || null;
}

function getSelectedItem(moduleName) {
    const id = getSelectedId(moduleName);
    if (!id) {
        return null;
    }

    const cacheMap = {
        'pagos': window.pagosCache,
        'tickets': window.ticketsCache,
        'roles': window.rolesCache,
        'usuarios': window.usuariosCache,
        'clientes': window.clientesCache,
        'tareas': window.tareasCache,
        'facturas': window.facturasCache,
        'planes': window.planesCache,
        'contratos': window.contratosCache,
        'nodos': window.nodosCache,
        'olts': window.oltsCache,
        'naps': window.napsCache,
        'equipos': window.equiposCache
    };

    const cache = cacheMap[moduleName];
    if (!cache) {
        return null;
    }
    ...
    const idField = idFieldMap[moduleName] || 'id';
    return (Array.isArray(cache) ? cache : []).find(item => Number(item[idField]) === Number(id)) || null;
}

function updateToolbarButtons(moduleName) {
    const hasSelection = !!getSelectedId(moduleName);
    const selectedItem = getSelectedItem(moduleName);

    document.querySelectorAll(`.toolbar-btn[data-module="${moduleName}"]`).forEach(btn => {
        const action = btn.dataset.action || '';

        if (action === 'edit' || action === 'delete' || action === 'reset') {
        ...
        if (action === 'validar' || action === 'rechazar' || action === 'cerrar') {
            if (!hasSelection) {
                btn.disabled = true;
                return;
            }

            if (moduleName === 'pagos' && selectedItem) {
                const estado = String(selectedItem.estado || '').toLowerCase();
                const isAdmin = isAdminRole();
                btn.disabled = !(estado === 'pendiente' && isAdmin);
                return;
            }

            if (moduleName === 'tickets' && selectedItem) {
                const estado = String(selectedItem.estado || '').toLowerCase();
                btn.disabled = estado === 'cerrado';
                return;
            }
...
function paginationControls(moduleName, data, currentPage, pageSize, renderFn) {
    const totalPages = Math.max(1, Math.ceil((Array.isArray(data) ? data.length : 0) / pageSize));
    const safePage = Math.min(Math.max(1, Number(currentPage) || 1), totalPages);

    return `
        <div class="pagination-controls">
            <button type="button" class="toolbar-btn" data-module="${moduleName}" data-action="prev" onclick="${typeof renderFn === 'function' ? renderFn.toString().replace(/\(page\s*=\s*1\)/, `(${Math.max(1, safePage - 1)})`) : ''}" ${safePage <= 1 ? 'disabled' : ''}>Anterior</button>
            <span>Página ${safePage} / ${totalPages}</span>
            <button type="button" class="toolbar-btn" data-module="${moduleName}" data-action="next" onclick="${typeof renderFn === 'function' ? renderFn.toString().replace(/\(page\s*=\s*1\)/, `(${Math.min(totalPages, safePage + 1)})`) : ''}" ${safePage >= totalPages ? 'disabled' : ''}>Siguiente</button>
        </div>
    `;
}

function searchFilter(moduleName, data, searchFields = []) {
    const input = document.getElementById(`${moduleName}-search`);
...
function buildSessionFetchHeaders(extraHeaders = {}) {
...
function renderMenu(autoNavigate = true) {
    ...
    const normalizePageId = (value) => {
        ...
        const aliases = {
            ...
            'configuración': 'roles',
            'configuracion': 'roles',
            'administración y accesos': 'roles',
            'administracion y accesos': 'roles',
            'roles': 'roles',
            'modulos': 'modulos',
            'módulos': 'modulos',
            'empresa': 'empresa',
    ...
    if (isAdminRole()) {
        const configEntries = [
            { page: 'roles', label: 'Roles', module: 'Configuración' },
            { page: 'modulos', label: 'Módulos', module: 'Configuración' },
            { page: 'empresa', label: 'Empresa', module: 'Configuración' },
...
function renderProfileMenu() {
    ...
    menu.innerHTML = `
        <button type="button" onclick="selectProfileMenuPage('perfil')">Mi perfil</button>
...
function applyEmpresaConfig(config = {}) {
    ...
    const empresaPreview = document.getElementById('empresa-logo-preview');
    ...
    if (empresaPreview) {
        empresaPreview.src = logoPath;
    }
}

async function cargarConfiguracionEmpresa() {
    try {
        const response = await fetch('api_empresa.php', {
            headers: buildSessionFetchHeaders()
        });
        ...
        if (!result || result.status !== 'success' || !result.config) {
            ...
            if (feedback) {
                ...
                feedback.textContent = result?.message || 'No se pudo cargar la configuración de la empresa.';
            }
            console.warn('No se pudo cargar la configuración de empresa:', result?.message || 'Respuesta inválida');
            return;
        }

        const config = result.config;
        const nombreInput = document.getElementById('empresa-nombre');
        const colorInput = document.getElementById('empresa-color');

        if (nombreInput) {
            nombreInput.value = config.nombre_empresa || '';
        }
        if (colorInput) {
            colorInput.value = config.color_primario || '#2563eb';
        }

        applyEmpresaConfig(config);
    } catch (error) {
        const feedback = document.getElementById('empresa-feedback');
        if (feedback) {
            feedback.style.color = 'red';
            feedback.textContent = 'No se pudo cargar la configuración de la empresa.';
        }
...
async function guardarEmpresa() {
    ...
    const color = document.getElementById('empresa-color')?.value || '#2563eb';
    const fileInput = document.getElementById('empresa-logo');
    const feedback = document.getElementById('empresa-feedback');
    const formData = new FormData();

    formData.append('nombre_empresa', nombre);
    formData.append('color_primario', color);
    ...
    try {
        ...
        if (feedback) {
            ...
            feedback.textContent = result.message || 'Operación realizada';
        }
        ...
        if (feedback) {
            feedback.style.color = 'red';
            feedback.textContent = 'No se pudo guardar la empresa.';
        }
        console.error(error);
    }
}
...
async function renderPerfil() {
    try {
        ...
        const sesiones = Array.isArray(result.sesiones) ? result.sesiones : [];
        ...
        // IDs Nuevos (Carnet)
        const avatarEl = document.getElementById('perfil-avatar-iniciales');
        const usernameLargeEl = document.getElementById('perfil-username-large');
        const badgeEl = document.getElementById('perfil-rol-badge');
        
        const tbody = document.getElementById('perfil-sesiones-body');
        const pagination = document.getElementById('perfil-sesiones-pagination');

        const nombreUser = usuario.username || 'Usuario';
        
        if (usernameEl) usernameEl.textContent = nombreUser;
        if (emailEl) emailEl.textContent = usuario.email || '—';
        if (rolEl) rolEl.textContent = usuario.nombre_rol || '—';
        
        if (avatarEl) avatarEl.textContent = nombreUser.charAt(0).toUpperCase();
        if (usernameLargeEl) usernameLargeEl.textContent = nombreUser;
        if (badgeEl) badgeEl.textContent = usuario.nombre_rol || 'Sin rol';

        if (tbody) {
            tbody.innerHTML = sesiones.length ? sesiones.map(sesion => `
                <tr>
                    <td>${escapeHtml(String(sesion.id_sesion || ''))}</td>
                    <td>${escapeHtml(String(sesion.creado_en || ''))}</td>
                    <td>${escapeHtml(String(sesion.expira_en || ''))}</td>
                </tr>
            `).join('') : '<tr><td colspan="3">No hay sesiones registradas.</td></tr>';
        }
    } catch (error) {
...
async function showPage(pageId, element) {
    ...
    if ((pageId === 'roles' || pageId === 'modulos' || pageId === 'empresa' || pageId === 'usuarios') && !isAdminRole()) {
        return;
    }
    ...
    const titles = {
        dash: { title: 'Dashboard', eyebrow: 'Análisis' },
        clientes: { title: 'Clientes', eyebrow: 'Gestión' },
        tareas: { title: 'Control de tareas', eyebrow: 'Control' },
        tickets: { title: 'Tickets', eyebrow: 'Soporte' },
        mapa: { title: 'Mapa técnico', eyebrow: 'Infraestructura' },
        roles: { title: 'Roles', eyebrow: 'Configuración' },
        modulos: { title: 'Módulos y permisos', eyebrow: 'Configuración' },
        empresa: { title: 'Empresa', eyebrow: 'Configuración' },
        perfil: { title: 'Mi perfil', eyebrow: 'Configuración' },
        usuarios: { title: 'Usuarios', eyebrow: 'Configuración' },
        password: { title: 'Cambiar contraseña', eyebrow: 'Cuenta' },
        reportes: { title: 'Reportes', eyebrow: 'Analítica' },
        nodos: { title: 'Nodos', eyebrow: 'Infraestructura' },
        olts: { title: 'OLTs', eyebrow: 'Infraestructura' },
        naps: { title: 'NAPs', eyebrow: 'Infraestructura' },
        equipos: { title: 'Equipos', eyebrow: 'Infraestructura' },
        facturas: { title: 'Facturación', eyebrow: 'Finanzas' },
        pagos: { title: 'Pagos', eyebrow: 'Finanzas' },
        planes: { title: 'Planes', eyebrow: 'Finanzas' },
        contratos: { title: 'Contratos', eyebrow: 'Finanzas' }
    };

    const pageInfo = titles[pageId] || { title: 'Panel', eyebrow: 'Operación' };
    const title = document.getElementById('page-title');
    const eyebrowEl = document.getElementById('page-eyebrow');
    if (title) title.textContent = pageInfo.title;
    if (eyebrowEl) eyebrowEl.textContent = pageInfo.eyebrow;
    ...
    initToolbar(pageId, {
        searchId: pageId === 'clientes' ? 'search-client' : pageId === 'usuarios' ? 'users-search' : pageId === 'roles' ? 'roles-search' : pageId === 'tareas' ? 'task-search' : pageId === 'tickets' ? 'ticket-search' : ''
    });
    ...
    if (pageId === 'modulos') {
        const select = document.getElementById('modulos-role-select');
        if (select && select.value) {
            loadRolePermissions('modulos');
        }
    }
...
function renderUsersPage(page = 1) {
    ...
    tbody.innerHTML = visibleUsers.length ? visibleUsers.map(usuario => {
        const estado = usuario.estado || 'Activo';
        ...
        return `
            <tr data-module-row="usuarios" data-id="${escapeHtml(userId)}" onclick="selectRow('usuarios', '${escapeHtml(userId)}', this)">
                <td><strong>${escapeHtml(usuario.username || 'Usuario')}</strong></td>
...
function renderRoleList(roles) {
    ...
    const rolesArray = Array.isArray(roles) ? roles : (Array.isArray(window.rolesCache) ? window.rolesCache : []);
    const search = document.getElementById('roles-search')?.value.trim().toLowerCase() || '';

    const filteredRoles = rolesArray.filter(role => {
        if (!search) return true;
        return String(role.nombre_rol || '').toLowerCase().includes(search);
    });

    if (!filteredRoles.length) {
        container.innerHTML = '<tr><td colspan="2">No hay roles que coincidan con la búsqueda.</td></tr>';
        return;
    }

    container.innerHTML = filteredRoles.map(role => {
        const roleId = String(role.id_rol);
        return `
            <tr data-module-row="roles" data-id="${escapeHtml(roleId)}" onclick="selectRow('roles', '${escapeHtml(roleId)}', this)">
                <td><strong>${escapeHtml(roleId)}</strong></td>
                <td>${escapeHtml(role.nombre_rol)}</td>
            </tr>
        `;
    }).join('');
}
...
function selectRole(roleId) {
    ...
    window.selectedIds.roles = String(roleId);
    configSelect.value = String(roleId);
    updateToolbarButtons('roles');
    loadRolePermissions('config');
}

function preseleccionarRolEnModulos(roleId) {
    const select = document.getElementById('modulos-role-select');
    if (!select) {
        return;
    }
    if (!select.querySelector(`option[value="${roleId}"]`)) {
        loadRoles().then(() => {
            const refreshedSelect = document.getElementById('modulos-role-select');
            if (refreshedSelect) {
                refreshedSelect.value = String(roleId);
                loadRolePermissions('modulos');
            }
        });
        return;
...
async function guardarUsuarioEditado() {
    ...
    try {
        const response = await fetch('usuarios_editar.php', {
            method: 'PUT',
            headers: buildSessionFetchHeaders({ 'Content-Type': 'application/json' }),
...
function renderTasksPage(page = 1) {
    ...
    const search = (document.getElementById('tareas-search')?.value || '').toLowerCase();
    ...
    const totalPages = Math.max(1, Math.ceil(filteredTareas.length / window.taskPageSize));
    const safePage = Math.min(Math.max(1, Number(page) || 1), totalPages);
    window.currentTasksPage = safePage;

    const start = (safePage - 1) * window.taskPageSize;
    const visibleTasks = filteredTareas.slice(start, start + window.taskPageSize);

    container.innerHTML = visibleTasks.length ? visibleTasks.map(tarea => `
            <tr data-module-row="tareas" data-id="${escapeHtml(String(tarea.id_tarea || ''))}" onclick="selectRow('tareas', '${escapeHtml(String(tarea.id_tarea || ''))}', this)">
                <td>
...
function renderTicketsPage(page = 1) {
    ...
    const filteredTickets = tickets.filter(ticket => {
        const matchStatus = filter === 'Todos' || String(ticket.estado || '').toLowerCase() === String(filter || '').toLowerCase();
        const matchSearch = !search ||
            (ticket.asunto || '').toLowerCase().includes(search) ||
            (ticket.descripcion || '').toLowerCase().includes(search) ||
            (ticket.nombres || '').toLowerCase().includes(search) ||
            (ticket.apellidos || '').toLowerCase().includes(search) ||
            (ticket.creador || '').toLowerCase().includes(search);
    ...
    container.innerHTML = visibleTickets.length ? visibleTickets.map(ticket => `
            <tr data-module-row="tickets" data-id="${escapeHtml(String(ticket.id_ticket || ''))}" data-ticket-status="${escapeHtml(ticket.estado || 'Abierto')}" onclick="selectRow('tickets', '${escapeHtml(String(ticket.id_ticket || ''))}', this)">
                <td>
...
async function eliminarTarea(id) {
    ...
    const response = await fetch('api_tareas.php?id=' + id, {
        method: 'DELETE',
        headers: buildSessionFetchHeaders()
    });
    const result = await response.json();
...
async function eliminarTicket(id) {
    ...
    const response = await fetch('api_tickets.php?id=' + id, {
        method: 'DELETE',
        headers: buildSessionFetchHeaders()
    });
    const result = await response.json();
...
function updateDashboardData() {
    ...
    if (incomeCanvas) {
        ...
        if (periods.length >= 2 && dataSegments.some(value => value > 0)) {
            const accentRgb = getComputedStyle(document.documentElement).getPropertyValue('--accent-rgb').trim() || '37, 99, 235';
...
async function loadClientes(filter = '', page = 1) {
    ...
    try {
        ...
        list.innerHTML = clientes.map(cliente => `
            <tr data-module-row="clientes" data-id="${escapeHtml(String(cliente.id_cliente || ''))}" data-client-id="${cliente.id_cliente}" onclick="selectRow('clientes', '${escapeHtml(String(cliente.id_cliente || ''))}', this)">
...
async function loadRoles() {
    ...
    const configSelect = document.getElementById('config-role-select');
    const modulosSelect = document.getElementById('modulos-role-select');
    ...
    try {
        ...
        if (modulosSelect) modulosSelect.innerHTML = options;
        ...
        if (roles.length > 0 && modulosSelect) {
            loadRolePermissions('modulos');
        }
    } catch (error) {
        ...
        if (modulosSelect) modulosSelect.innerHTML = '<option value="1">Administrador</option>';
...
async function crearRol() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            if (typeof toggleRoleModal === 'function') setTimeout(() => toggleRoleModal(true), 1000);
        }
    } catch (error) {
        if (feedback) {
            feedback.style.color = 'red';
...
async function crearUsuario() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            await loadUsers();
            await loadRoles();
            if (typeof toggleUserModal === 'function') setTimeout(() => toggleUserModal(true), 1500);
        }
    } catch (error) {
        feedback.style.color = 'red';
...
async function loadRolePermissions(target = 'default') {
    const selectId = target === 'config' ? 'config-role-select' : target === 'modulos' ? 'modulos-role-select' : 'role-select';
    const containerId = target === 'config' ? 'config-permissions-list' : target === 'modulos' ? 'modulos-permissions-list' : 'permissions-list';
    const select = document.getElementById(selectId);
    ...
    try {
        ...
        container.innerHTML = `<div class="permissions-matrix">` + result.modulos.map(modulo => `
            <div class="permission-module-card">
                <h4 class="permission-module-title">${escapeHtml(modulo.nombre_modulo)}</h4>
                <div style="display:flex; flex-direction:column; gap:8px;">
                    ${modulo.vistas.map(vista => `
                        <div class="permission-item">
                            <span class="permission-label">${escapeHtml(vista.nombre_pagina)}</span>
...
async function guardarPermisos(target = 'default') {
    const selectId = target === 'config' ? 'config-role-select' : target === 'modulos' ? 'modulos-role-select' : 'role-select';
    const containerId = target === 'config' ? 'config-permissions-list' : target === 'modulos' ? 'modulos-permissions-list' : 'permissions-list';
    const feedbackId = target === 'config' ? 'config-permissions-feedback' : target === 'modulos' ? 'modulos-permissions-feedback' : 'permissions-feedback';
...
function renderNodosPage(page = 1) {
    ...
    const search = (document.getElementById('nodos-search')?.value || '').toLowerCase();
    const filteredNodos = nodos.filter(nodo => {
        if (!search) return true;
        return (nodo.nombre || '').toLowerCase().includes(search) ||
            (nodo.ubicacion || '').toLowerCase().includes(search);
    });
    const pageSize = 8;
    const totalPages = Math.max(1, Math.ceil(filteredNodos.length / pageSize));
    const safePage = Math.min(Math.max(1, Number(page) || 1), totalPages);
    window.currentNodosPage = safePage;

    const start = (safePage - 1) * pageSize;
    const visibleNodos = filteredNodos.slice(start, start + pageSize);

    container.innerHTML = visibleNodos.length ? visibleNodos.map(nodo => `
        <tr data-module-row="nodos" data-id="${escapeHtml(String(nodo.id_nodo || ''))}" onclick="selectRow('nodos', '${escapeHtml(String(nodo.id_nodo || ''))}', this)">
...
async function eliminarNodo(id) {
    ...
    const response = await fetch('api_nodos.php?id=' + id, {
        method: 'DELETE',
        headers: buildSessionFetchHeaders()
    });
    const result = await response.json();
...
function renderOltsPage(page = 1) {
    ...
    const olts = Array.isArray(window.oltsCache) ? window.oltsCache : [];
    const search = (document.getElementById('olts-search')?.value || '').toLowerCase();
    const filteredOlts = olts.filter(olt => {
        if (!search) return true;
        return (olt.codigo || '').toLowerCase().includes(search) ||
            (olt.marca_modelo || '').toLowerCase().includes(search) ||
            (olt.ip_gestion || '').toLowerCase().includes(search) ||
            (olt.nombre_nodo || '').toLowerCase().includes(search);
    });
    const pageSize = 8;
    const totalPages = Math.max(1, Math.ceil(filteredOlts.length / pageSize));
    const safePage = Math.min(Math.max(1, Number(page) || 1), totalPages);
    window.currentOltsPage = safePage;

    const start = (safePage - 1) * pageSize;
    const visibleOlts = filteredOlts.slice(start, start + pageSize);

    container.innerHTML = visibleOlts.length ? visibleOlts.map(olt => `
        <tr data-module-row="olts" data-id="${escapeHtml(String(olt.id_olt || ''))}" onclick="selectRow('olts', '${escapeHtml(String(olt.id_olt || ''))}', this)">
...
async function eliminarOlt(id) {
    ...
    const response = await fetch('api_olts.php?id=' + id, {
        method: 'DELETE',
        headers: buildSessionFetchHeaders()
    });
    const result = await response.json();
...
function renderNapsPage(page = 1) {
    ...
    const naps = Array.isArray(window.napsCache) ? window.napsCache : [];
    const search = (document.getElementById('naps-search')?.value || '').toLowerCase();
    const filteredNaps = naps.filter(nap => {
        if (!search) return true;
        return (nap.codigo || '').toLowerCase().includes(search) ||
            (nap.ubicacion_fisica || '').toLowerCase().includes(search) ||
            (nap.olt_codigo || '').toLowerCase().includes(search) ||
            (nap.olt_nombre || '').toLowerCase().includes(search) ||
            (nap.nodo_nombre || '').toLowerCase().includes(search);
    });
    const pageSize = 8;
    ...
    const start = (safePage - 1) * pageSize;
    const visibleNaps = filteredNaps.slice(start, start + pageSize);

    container.innerHTML = visibleNaps.length ? visibleNaps.map(nap => `
        <tr data-module-row="naps" data-id="${escapeHtml(String(nap.id_nap || ''))}" data-nap-id="${nap.id_nap}" onclick="selectRow('naps', '${escapeHtml(String(nap.id_nap || ''))}', this)">
...
function toggleRoleModal(forceClose = false, reset = true) {
    const modal = document.getElementById('role-modal');
    if (modal) {
        modal.style.display = forceClose ? 'none' : (modal.style.display === 'flex' ? 'none' : 'flex');
    }
    if (!forceClose && reset) {
        const inputName = document.getElementById('config-new-role-name');
...
async function eliminarNap(id) {
    ...
    const response = await fetch('api_naps.php?id=' + id, {
        method: 'DELETE',
        headers: buildSessionFetchHeaders()
    });
    const result = await response.json();
...
async function eliminarServicio(id) {
    ...
    try {
        const response = await fetch('api_servicios.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
...
function renderEquiposPage(page = 1) {
    ...
    const search = (document.getElementById('equipos-search')?.value || '').toLowerCase();
    const filteredEquipos = equipos.filter(equipo => {
        ...
        const searchable = [
            ...
            equipo.propiedad || '',
            `${equipo.nombres || ''} ${equipo.apellidos || ''}`.trim()
        ].join(' ').toLowerCase();
    ...
    const start = (safePage - 1) * pageSize;
    const visibleEquipos = filteredEquipos.slice(start, start + pageSize);

    container.innerHTML = visibleEquipos.length ? visibleEquipos.map(equipo => `
        <tr data-module-row="equipos" data-id="${escapeHtml(String(equipo.id_equipo || ''))}" onclick="selectRow('equipos', '${escapeHtml(String(equipo.id_equipo || ''))}', this)">
            ...
            <td>${escapeHtml(equipo.propiedad === 'cliente' ? 'Cliente' : 'Empresa')}</td>
            <td>${escapeHtml(equipo.id_servicio ? `${equipo.nombres || ''} ${equipo.apellidos || ''}`.trim() || 'Cliente' : 'Sin asignar')}</td>
            
        </tr>
    `).join('') : '<tr><td colspan="9">No hay equipos registrados.</td></tr>';
...
function toggleEquipoForm(forceClose = false, reset = true) {
    ...
    if (!forceClose && reset) {
        ...
        document.getElementById('equipo-propiedad').value = 'empresa';
...
async function guardarEquipo() {
    ...
    const propiedad = document.getElementById('equipo-propiedad').value;
...
function editarEquipo(id) {
    ...
    document.getElementById('equipo-propiedad').value = equipo.propiedad || 'empresa';
...
async function eliminarEquipo(id) {
    ...
    try {
        const response = await fetch('api_equipos.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
...
function renderFacturasPage(page = 1) {
    ...
    const facturas = Array.isArray(window.facturasCache) ? window.facturasCache : [];
    const search = (document.getElementById('facturas-search')?.value || '').toLowerCase();
    const filteredFacturas = facturas.filter(factura => {
        if (!search) return true;
        const cliente = `${factura.nombres || ''} ${factura.apellidos || ''}`.trim();
        return cliente.toLowerCase().includes(search) ||
            (factura.periodo || '').toLowerCase().includes(search) ||
            String(factura.monto || '').toLowerCase().includes(search) ||
            (factura.estado || '').toLowerCase().includes(search);
    });
    const totalPages = Math.max(1, Math.ceil(filteredFacturas.length / window.facturaPageSize));
    const safePage = Math.min(Math.max(1, Number(page) || 1), totalPages);
    window.currentFacturasPage = safePage;

    const start = (safePage - 1) * window.facturaPageSize;
    const visibleFacturas = filteredFacturas.slice(start, start + window.facturaPageSize);

    container.innerHTML = visibleFacturas.length ? visibleFacturas.map(factura => `
        <tr data-module-row="facturas" data-id="${escapeHtml(String(factura.id_factura || ''))}" onclick="selectRow('facturas', '${escapeHtml(String(factura.id_factura || ''))}', this)">
...
async function eliminarFactura(id) {
    ...
    try {
        const response = await fetch('api_facturas.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
...
function renderPagosPage(page = 1) {
    ...
    const pagos = Array.isArray(window.pagosCache) ? window.pagosCache : [];
    const search = (document.getElementById('pagos-search')?.value || '').toLowerCase();
    const filteredPagos = pagos.filter(pago => {
        if (!search) return true;
        const cliente = `${pago.nombres || ''} ${pago.apellidos || ''}`.trim();
        return cliente.toLowerCase().includes(search) ||
            (pago.factura_periodo || '').toLowerCase().includes(search) ||
            (pago.metodo_pago || '').toLowerCase().includes(search) ||
            String(pago.monto || '').toLowerCase().includes(search) ||
            (pago.estado || '').toLowerCase().includes(search);
    });
    const totalPages = Math.max(1, Math.ceil(filteredPagos.length / window.pagoPageSize));
    const safePage = Math.min(Math.max(1, Number(page) || 1), totalPages);
    window.currentPagosPage = safePage;

    const start = (safePage - 1) * window.pagoPageSize;
    const visiblePagos = filteredPagos.slice(start, start + window.pagoPageSize);
    ...
    container.innerHTML = visiblePagos.length ? visiblePagos.map(pago => {
        ...
        ` : '';
        return `
        <tr data-module-row="pagos" data-id="${escapeHtml(String(pago.id_pago || ''))}" onclick="selectRow('pagos', '${escapeHtml(String(pago.id_pago || ''))}', this)">
...
async function eliminarPago(id) {
    ...
    try {
        const response = await fetch('api_pagos.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
...
function renderPlanesPage(page = 1) {
    ...
    const search = (document.getElementById('planes-search')?.value || '').toLowerCase();
    const filteredPlanes = planes.filter(plan => {
        if (!search) return true;
        return (plan.nombre || '').toLowerCase().includes(search) ||
            (plan.velocidad || '').toLowerCase().includes(search) ||
            String(plan.precio_mensual || '').toLowerCase().includes(search) ||
            (plan.moneda || '').toLowerCase().includes(search);
    });
    const pageSize = 8;
    ...
    const start = (safePage - 1) * pageSize;
    const visiblePlanes = filteredPlanes.slice(start, start + pageSize);

    container.innerHTML = visiblePlanes.length ? visiblePlanes.map(plan => `
        <tr data-module-row="planes" data-id="${escapeHtml(String(plan.id_plan || ''))}" onclick="selectRow('planes', '${escapeHtml(String(plan.id_plan || ''))}', this)">
...
async function eliminarPlan(id) {
    ...
    try {
        const response = await fetch('api_planes.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
...
function renderContratosPage(page = 1) {
    ...
    const filteredContratos = contratos.filter(contrato => {
        if (!search) return true;
        const cliente = `${contrato.nombres || ''} ${contrato.apellidos || ''}`.trim();
        return cliente.toLowerCase().includes(search) ||
            (contrato.servicio_alias || contrato.direccion_texto || '').toLowerCase().includes(search) ||
            (contrato.tipo_contrato || '').toLowerCase().includes(search) ||
            (contrato.estado || '').toLowerCase().includes(search);
    });
    const pageSize = 8;
    ...
    const start = (safePage - 1) * pageSize;
    const visibleContratos = filteredContratos.slice(start, start + pageSize);

    container.innerHTML = visibleContratos.length ? visibleContratos.map(contrato => `
        <tr data-module-row="contratos" data-id="${escapeHtml(String(contrato.id_contrato || ''))}" onclick="selectRow('contratos', '${escapeHtml(String(contrato.id_contrato || ''))}', this)">
...
async function eliminarContrato(id) {
    ...
    try {
        const response = await fetch('api_contratos.php?id=' + id, {
            method: 'DELETE',
            headers: buildSessionFetchHeaders()
        });
        const result = await response.json();
