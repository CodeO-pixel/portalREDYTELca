...
if ($method === 'POST') {
    ...
    $propiedad = trim($input['propiedad'] ?? 'empresa');
    ...
    $propiedadesValidas = ['cliente', 'empresa'];
    if (!in_array($propiedad, $propiedadesValidas, true)) {
        $propiedad = 'empresa';
    }

    $stmtNap = $pdo->prepare("SELECT cantidad_puertos_max FROM naps WHERE id_nap = ?");
    $stmtNap->execute([$id_naps]);
    $napRow = $stmtNap->fetch(PDO::FETCH_ASSOC);
    if (!$napRow) {
        echo json_encode(['status' => 'error', 'message' => 'La NAP seleccionada no existe.']);
        exit;
    }
    if ($num_puerto_nap > (int) $napRow['cantidad_puertos_max']) {
        echo json_encode(['status' => 'error', 'message' => "El puerto {$num_puerto_nap} excede la capacidad física de esta NAP ({$napRow['cantidad_puertos_max']} puertos máximo)."]);
        exit;
    }

    try {
        $stmt = $pdo->prepare("INSERT INTO equipos (tipo, marca, modelo, direccion_mac, num_puerto_nap, estado_fisico, propiedad, id_naps, id_servicio)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $ok = $stmt->execute([$tipo, $marca ?: null, $modelo ?: null, $direccion_mac, $num_puerto_nap, $estado_fisico, $propiedad, $id_naps, $id_servicio]);
...
if ($method === 'PUT') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE equipos SET tipo = ?, marca = ?, modelo = ?, direccion_mac = ?, num_puerto_nap = ?, estado_fisico = ?, propiedad = ?, id_naps = ?, id_servicio = ? WHERE id_equipo = ?");
        $ok = $stmt->execute([$tipo, $marca ?: null, $modelo ?: null, $direccion_mac, $num_puerto_nap, $estado_fisico, $propiedad, $id_naps, $id_servicio, $id]);
